...
if [[ $LR_CHANNEL == "right" ]]; then
 ...
 # WE NEED TO FIND THE MOST RECENT FLDIGI LOG FILE
  ...
  #echo "YF="$SKIMMER_FILE
#echo -ne '\033]0;'Skimmer is $SKIMMER_FILE'\007'
#   echo "AC="$age_check
# echo $file_counter
...
setScreen # clears screen and window buffer
  ...
  # jump start on refresh cycle so header is only on 3 secs.
...
function check4OutGoing {
  ...
  currentSkimmerStamp=$(stat $SKIMMER_FILE -c %Y)
#echo a $currentSkimmerStamp
#echo b $lastSkimmerStamp
  if [[ currentSkimmerStamp -gt lastSkimmerStamp ]]
    then
      #echo file changed
#      sleep 10
      lastSkimmerStamp=$currentSkimmerStamp
...
#
#
# hisCall=$(echo $last_line | cut -d':' -f4)  #pull call sign out of string
# hisCall=$(echo $hisCall | xargs)  #strip off leading space
 fi
#echo "$hisCall"
